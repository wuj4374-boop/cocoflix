...
export async function createApp() {
  ...
  try {
    ...
    if (sourceCount === 0) {
      const seedLogger = app.get(WinstonLoggerService);
      seedLogger.log('检测到空数据库，开始自动初始化...', 'Bootstrap');
      ...
      try {
        ...
        seedLogger.log('管理员账号已创建: admin / admin123', 'Bootstrap');
      } catch (e) { seedLogger.error('创建管理员失败: ' + e, 'Bootstrap'); }
      ...
      try {
        ...
        seedLogger.log(`${genres.length} 个默认分类已创建`, 'Bootstrap');
      } catch (e) { seedLogger.error('创建分类失败: ' + e, 'Bootstrap'); }
      ...
      try {
        ...
        seedLogger.log(`${sources.length} 个资源站已创建`, 'Bootstrap');
      } catch (e) { seedLogger.error('创建资源站失败: ' + e, 'Bootstrap'); }

      // 显式保存数据库（sql.js autoSave 可能不可靠）
      try {
        const driver = dataSource.driver as any;
        if (typeof driver.save === 'function') {
          await driver.save();
          seedLogger.log('数据库已显式保存到磁盘', 'Bootstrap');
        }
      } catch (e) {
